...
function(TGE_MODULE name)
    ...
    if(TGE_MOD_PRIVATE_INCLUDE_DIR AND EXISTS "${TGE_MOD_PRIVATE_INCLUDE_DIR}")
        ...
        set_property(GLOBAL APPEND PROPERTY TGE_PRIVATE_INCLUDE_DIRS "${TGE_MOD_PRIVATE_INCLUDE_DIR}")
    endif()
    ...
    # Shared aggregates reuse these object files. Mark runtime definitions for
    # export while keeping import semantics off static-library consumers.
    if(TGE_BUILD_SHARED)
        target_compile_definitions(
            ${_obj_target}
            PRIVATE TGE_SHARED BUILDING_TGE)
    endif()
...
function(TGE_TEST_MODULE module)
    ...
    if(unit_sources)
        if(NOT TGE_ENABLE_TESTS)
            ...
            if(_tge_priv_inc_dirs)
                list(REMOVE_DUPLICATES _tge_priv_inc_dirs)
                target_include_directories(${test_target} PRIVATE ${_tge_priv_inc_dirs})
            endif()
    ...
    if(benchmark_sources)
        if(NOT TGE_ENABLE_BENCHMARKS)
            ...
            if(_tge_priv_inc_dirs)
                list(REMOVE_DUPLICATES _tge_priv_inc_dirs)
                target_include_directories(${benchmark_target} PRIVATE ${_tge_priv_inc_dirs})
            endif()
